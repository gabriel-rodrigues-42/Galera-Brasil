...
**Companion doc:** `game.md` holds the vision/GDD; this file holds the *how and in what order*.
...
1. **The fun core doesn't need the hard tech.** "Walking the feed" — exploring a 3D praça
   ...
   server. P2P, escrow, and peer-validated anti-cheat are all *optimizations or values*,
...
- **Demo:** *your own* profile as a walkable 3D hub, live at a link.
...
Goal: see other people. This is where it becomes a *social* space.
...
Goal: the economy's *discovery* half, with zero money handling.
- [ ] Storefront hub template: portfolio display, service list with prices as *information*
...
- [ ] Measure server costs. *If and only if* bandwidth hurts: move avatar position sync
