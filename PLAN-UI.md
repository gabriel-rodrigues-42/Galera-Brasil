...
- [x] `<interact-hint>`: single `setText(text)` method (empty string hides via a `.visible` class flip — inverted from the legacy `.hidden` class but behaviorally identical).
- [x] `<debug-panel>`: `setLog(text)`, `setStats(text)`, `getLogText()`, `setCollapsed(bool)`/`toggleCollapsed()`/`expand()`. Implements a new `DebugPanelSink` interface from `logger.ts` — `initDebugPanel(panel: DebugPanelSink)` now takes the component itself instead of two raw `<pre>` elements, so `log()`/`updateStats()` never touch DOM directly. The badge's click-to-toggle is now internal to the component (main.ts no longer has a `debugBadgeEl.addEventListener`); the `~`-key and Ctrl+Shift+D handlers in main.ts call `debugPanelEl.toggleCollapsed()` / `.expand()` + `.getLogText()`.
- [x] Enabled `html { font-size: clamp(14px, 1.2vw, 18px) }` — verified live (`getComputedStyle(document.documentElement).fontSize` → a viewport-dependent value, not a fixed 16px).
- [x] `style.css` down to 80 lines (reset, `html/body`, `#app`, `#scene`, `bite-shake` keyframes, `#blackout-vignette`). Slightly over the ~60-line estimate because `#blackout-vignette` (a Release-3.1 sabotage effect, added after this plan was written) legitimately stays global chrome alongside `#scene`/`#app`. Also swept two pieces of dead CSS found along the way: an `overlayFloat` keyframe with zero remaining references, and a `@media (max-width: 480px)` block styling `#overlay`/`.overlay-card`/`#join-form` — all IDs that stopped existing back in Phase 5 (join-overlay already has its own responsive `@media` inside its shadow sheet).
- [x] Sweep confirmed: every `document.querySelector` in main.ts is now either `#scene`, `#blackout-vignette` (plain div, a controller dep, same tier as `#scene`/`#app`), or a typed component handle; `pnpm knip` clean; DESIGN.md §11 status table all ✅ (also fixed gotcha #9, which referenced a `ui-state.anyModalOpen()` that was never built — Phase 1 already noted `ui-state.ts` moves out for lack of a consumer, and no controller since has needed one; documented the real pattern instead: each controller tracks its own open/closed state via a closure-scoped getter, and every key-routing guard in main.ts checks them individually).
- [x] Regression pass on the running dev server: fresh-tab console clean; join flow; GM panel open/close via `panel-close`; battle-hud XP float; chat append; NPC panel open — all coexisting with no errors. `pnpm build` and a `pnpm preview` boot both smoke-tested clean.

**End-state vs. original estimate:** `index.html` 54 lines (~60 target, hit), `style.css` 80 lines (~60 target, close — see above), `client/src/ui/` 25 components + 6 controllers (~20 files estimated, exceeded because Phase 7's estimate predates the 3.x Mutirão work, which added its own components/controllers). `main.ts` is 1,988 lines, well over the original 1,100–1,300 estimate — that estimate was written before Release 3.0-3.2 existed; the gap is debris interaction, minigame wiring, and `game-controller`/`npc-panel-controller`/`chat-controller` hookup, not leftover legacy cruft the UI refactor failed to remove.
